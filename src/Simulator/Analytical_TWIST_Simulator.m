...
breastPhantomParams.lesions = [ ...
    struct('center_mm', [-130 * voxel_size_mm(1), 0, 0], ...
        'radius_mm', 8, 'intensityFunction', sharedLesionIntensityFunction), ...
    struct('center_mm', [18 * voxel_size_mm(1), 30 * voxel_size_mm(2), 16 * voxel_size_mm(3)], ...
        'radius_mm', 4, 'intensityFunction', sharedLesionIntensityFunction), ...
    struct('center_mm', [-10 * voxel_size_mm(1), -10 * voxel_size_mm(2), -14 * voxel_size_mm(3)], ...
        'radius_mm', 2, 'intensityFunction', sharedLesionIntensityFunction), ...
    struct('center_mm', [6 * voxel_size_mm(1), -20 * voxel_size_mm(2), 22] * voxel_size_mm(3), ...
        'radius_mm', 1, 'intensityFunction', sharedLesionIntensityFunction)];
...
lesion_centers = [68, 179, 121; 38, 31, 105; 78, 59, 135; 88, 43, 99];
lesion_radii = [5; 2.5; 1; 0];
roiLinearIdxByLesion = buildLesionRoiIndices(IMmatrix_crop_size, lesion_centers, lesion_radii);
...
function roiLinearIdxByLesion = buildLesionRoiIndices(IMmatrix_crop_size, lesion_centers, lesion_radii)
% buildLesionRoiIndices  Precompute linear indices for spherical lesion ROIs.

num_lesions = numel(lesion_radii);
[X, Y, Z] = ndgrid( ...
    1:IMmatrix_crop_size(1), ...
    1:IMmatrix_crop_size(2), ...
    1:IMmatrix_crop_size(3));

roiLinearIdxByLesion = cell(1, num_lesions);
for ii = 1:num_lesions
    squared_dists = ...
        (X - lesion_centers(ii, 1)).^2 + ...
        (Y - lesion_centers(ii, 2)).^2 + ...
        (Z - lesion_centers(ii, 3)).^2;
    roiLinearIdxByLesion{ii} = find(squared_dists <= lesion_radii(ii)^2);
end
end

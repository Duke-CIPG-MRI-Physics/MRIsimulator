...
% MATLAB tables do not transition cleanly to the GPU. We extract vectors here:
gpu_Freq = gpuArray(Sampling_Table.Frequency);
gpu_Phase = gpuArray(Sampling_Table.("Row (phase)"));
gpu_Slice = gpuArray(Sampling_Table.("Column (slice)"));
gpu_Timing = gpuArray(Sampling_Table.Timing);
gpu_Frame = gpuArray(Sampling_Table.Frame);

k_spatFreq_xyz_gpu = gpuArray(k_spatFreq_xyz);
%% ==========================================================
...
% Preallocate the final image array natively on the GPU
padsize = matrix_size_complete(fps_to_xyz) - matrix_size_acquired(fps_to_xyz);
twistImage_gpu = zeros([matrix_size_complete(fps_to_xyz), nTimes], 'gpuArray');

% Initialize variable for view sharing 
previousKspace_gpu = gpuArray([]); 

for iTime = 1:nTimes
        
    % 1. Calculate current K-space Samples and put points in correct locations
    currentMask = (gpu_Frame == (iTime - 1));
    currentKspace = nan(matrix_size_acquired, 'gpuArray');
    
    currentIdx = sub2ind(matrix_size_acquired, ...
        gpu_Freq(currentMask), ...
        gpu_Phase(currentMask), ...
        gpu_Slice(currentMask));
        
    currentKspace(currentIdx) = phantom.kspaceAtTime(...
        k_spatFreq_xyz_gpu(1, currentMask), ...
        k_spatFreq_xyz_gpu(2, currentMask), ...
        k_spatFreq_xyz_gpu(3, currentMask), ...
        gpu_Timing(currentMask)', ...
        maxChunkSize)';
    
    % 2. Apply View Sharing (Fill in missing k-space points if pB is active)
    if pB ~= 0 && iTime > 1
        missingKspaceData = isnan(currentKspace);
        currentKspace(missingKspaceData) = previousKspace_gpu(missingKspaceData);
    end
    ...
    twistImage_gpu(:,:,:,iTime) = fftshift(ifftn(ifftshift(paddedKspace)));
    
    % 5. Prepare for the next TWIST frame
    if pB ~= 0
        previousKspace_gpu = currentKspace;
    end
end

% Post-processing dimensions and intensity on GPU
twistImage_gpu = permute(twistImage_gpu, [2, 1, 3, 4]);
voxel_volume = prod(voxel_size_mm);
twistImage_gpu = twistImage_gpu ./ voxel_volume;

%% --- Resolving Oversampling on GPU
crop_amount = matrix_size_complete-IMmatrix_crop_size;
margin = floor(crop_amount ./ 2); 
final_IMspace_gpu = twistImage_gpu(...
    margin(1)+1 : margin(1)+IMmatrix_crop_size(1), ...
    margin(2)+1 : margin(2)+IMmatrix_crop_size(2), ...
    margin(3)+1 : margin(3)+IMmatrix_crop_size(3),...
    :);

% Transfer back to CPU memory for struct assignment and output
final_IMspace = gather(final_IMspace_gpu);

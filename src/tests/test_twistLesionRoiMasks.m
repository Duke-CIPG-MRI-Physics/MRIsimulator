...
%   the expected radius shrink and one-voxel fallback behavior.
    ...
    expectedRoiRadii_mm = [7; 3; 1; 0];
    assert(max(abs(roiInfo.roiRadius_mm - expectedRoiRadii_mm)) < 1e-12, ...
        'Expected ROI radii to shrink by lesionBorder_mm.');
    assert(roiInfo.usedSingleVoxelFallback(4), ...
        'Expected the XS lesion ROI to fall back to a single voxel.');
    assert(numel(roiInfo.linearIdxByLesion{4}) == 1, ...
        'Expected the XS lesion ROI fallback to produce exactly one voxel.');
end

...
heart = AnalyticalEllipsoid3D(heart_a_mm, heart_b_mm, heart_c_mm, 1, ...
    heart_center, [heart_roll_deg, heart_pitch_deg, heart_yaw_deg]);
...
rightLung = AnalyticalEllipsoid3D(lung_a_mm, lung_b_mm, lung_c_mm, 0.1, ...
    center_R_mm, [lung_roll_R, lung_pitch_R, lung_yaw_R]);
...
leftLung = AnalyticalEllipsoid3D(lung_a_mm, lung_b_mm, lung_c_mm, 0.1, ...
    center_L_mm, [lung_roll_L, lung_pitch_L, lung_yaw_L]);
...
fat_outer = AnalyticalEllipticalCylinder3D(patientWidth_outer_mm, patientThickness_outer_mm, 0.9*FOV_mm(3), ...
    [], bodyCenter, [fat_roll_deg, fat_pitch_deg, fat_yaw_deg]);
...
fat_inner = AnalyticalEllipticalCylinder3D(patientWidth_inner_mm, patientThickness_inner_mm, 0.9*FOV_mm(3), ...
    [], bodyCenter, [fat_roll_deg, fat_pitch_deg, fat_yaw_deg]);
...
breast_right = AnalyticalCylinder3D(breast_radius_mm, breast_depth_mm, [], ...
    right_breast_center, [breast_roll_deg, breast_pitch_deg, breast_yaw_deg]);
...
breast_left = AnalyticalCylinder3D(breast_radius_mm, breast_depth_mm, 0.5, ...
    left_breast_center, [breast_roll_deg, breast_pitch_deg, breast_yaw_deg]);

% A/P blood vessel
ap_vessel_roll_deg = breast_roll_deg;
ap_vessel_pitch_deg = breast_pitch_deg;
ap_vessel_yaw_deg = breast_yaw_deg;
...
enhancedCenter = [right_breast_center(1), ...
    right_breast_center(2)  - 0.5*unenhancedLength, ...
    right_breast_center(3)];
unenhancedCenter = [right_breast_center(1), ...
    right_breast_center(2) + 0.5*enhancedLength, ...
    right_breast_center(3)];

vessel_ap_unenhanced = AnalyticalCylinder3D(vessel_radius_mm, unenhancedLength, 0.4, ...
    unenhancedCenter, [ap_vessel_roll_deg, ap_vessel_pitch_deg, ap_vessel_yaw_deg]);
...
K = fatComposite.kspace(kx, ky, kz) + ...
    ...
    heart.kspace(kx, ky, kz) + ...
    rightLung.kspace(kx, ky, kz) + ...
    leftLung.kspace(kx, ky, kz) + ...
    breast_left.kspace(kx, ky, kz) + ...
    breastRightTissue.kspace(kx, ky, kz) + ...
    vessel_ap_unenhanced.kspace(kx, ky, kz) + ...
    vessel_ap_enhanced.kspace(kx, ky, kz);
